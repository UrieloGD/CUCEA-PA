...
class TabulatorEditManager {
    constructor() {
      ...
      // Mapeo de columnas para la tabla coord_per_prof
this.columnMap = {
    ID: "ID",
    Datos: "DATOS",
    Codigo: "CODIGO",
    ...
    waitForTable() {
      ...
      const checkTable = () => {
        if (window.table && typeof window.table.on === "function") {
          ...
          console.log("[TabulatorEditManager] Tabla y eventos listos!"); // ✅ Verificar en consola
        } else {
    ...
    setupTableEvents() {
      ...
      this.table.on("cellEdited", (cell) => {
        console.log('[TabulatorEditManager] Celda editada:', cell.getField(), cell.getValue());
        if (!this.puedeEditar) {
    ...
    handleCellEdit(cell) {
      console.log("[TabulatorEditManager] handleCellEdit llamado"); // ✅ Ver si se ejecuta
