document.addEventListener("DOMContentLoaded", function () {
  ...
  const columns = [
    ...
    {
      title: "HORAS DEFINITIVAS",
      field: "Horas_definitivas",
      editor: isEditable ? "input" : false,
      variableHeight: true,
    },
    {
      title: "UDG VIRTUAL CIT OTRO CENTRO",
    ...
    {
      title: "INVESTIGADOR POR NOMBRAMIENTO O CAMBIO DE FUNCION",
    ...
    {
      title: "CAMBIO DEDICACION DE PLAZA DOCENTE A INVESTIGADOR",
      field: "Cambio_dedicacion",
      editor: isEditable ? "input" : false,
      variableHeight: true,
    },
    {
      title: "TELEFONO PARTICULAR",
      field: "Telefono_particular",
      editor: isEditable ? "input" : false,
      variableHeight: true,
    },
    {
      title: "TELEFONO OFICINA O CELULAR",
    ...
    {
      title: "INSTITUCION",
      field: "Otro_institucion",
      editor: isEditable ? "input" : false,
      variableHeight: true,
    },
    {
      title: "ESTADO/PAIS",
      field: "Otro_estado_pais",
      editor: isEditable ? "input" : false,
      variableHeight: true,
    },
    {
  ...
  if (!tableElement) {
    console.error(
      'Error: No se encontró el elemento con ID "tabla-datos-tabulator"'
    );
  ...
  const table = new Tabulator("#tabla-datos-tabulator", {
    ajaxURL: "./functions/coord-personal-plantilla/get_data.php",
    ajaxConfig: "GET",
    ajaxParams: {},
    ajaxConfig: {
      ajaxLoader: false,
    ...
    ajaxURLGenerator: function (url, config, params) {
      ...
      if (params.sort && params.sort.length) {
        ajaxURL +=
          "sort=" + params.sort[0].field + "&dir=" + params.sort[0].dir + "&";
      }
    ...
    ajaxError: function (xhr, textStatus, errorThrown) {
      ...
      Swal.fire(
        "Error",
        "No se pudieron cargar los datos. Detalles: " + errorThrown,
        "error"
      );
    },
    tableBuilt: function () {
    ...
    columnResized: function (column) {
      ...
      console.log(
        `Columna ${column.getField()} redimensionada a ${column.getWidth()}px`
      );
    },
  ...
  function adjustColumnWidths() {
    ...
    table.columnManager.columns.forEach((column) => {
      ...
      let calculatedWidth =
        Math.max(headerLength, maxContentLength) * charWidth + padding;
  ...
  function applyBulmaClasses() {
    ...
    document
      .querySelectorAll(".tabulator-paginator button")
      .forEach((button) => {
