function mostrarPopupColumnas() {
    ...
    selectAllLabel.appendChild(document.createTextNode('Seleccionar/Deseleccionar Todas'));
    selectAllDiv.appendChild(selectAllCheckbox);
    selectAllDiv.appendChild(selectAllLabel);
    opcionesColumnas.appendChild(selectAllDiv);

    // Obtener los encabezados de la tabla
    var headers = Array.from(document.querySelectorAll('#tabla-datos th')).slice(2); // Ignorar la columna de checkbox y ID
    ...
    selectAllCheckbox.addEventListener('change', function() {
        var checkboxes = document.querySelectorAll('.columns-container input[type="checkbox"]');
...
function descargarExcelSeleccionado() {
    var columnasSeleccionadas = Array.from(document.querySelectorAll('#opciones-columnas .columns-container input[type="checkbox"]:checked'))
