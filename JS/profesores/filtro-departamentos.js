document.addEventListener("DOMContentLoaded", function () {
  ...
  selectAllLi.innerHTML =
    '<input type="checkbox" id="select-all"/>Todos los departamentos';
  dropdownList.insertBefore(selectAllLi, dropdownList.firstChild);

  // Get all department checkboxes
  const departmentCheckboxes = dropdownList.querySelectorAll(
  ...
  function updateTable() {
    ...
    if (selectAllCheckbox.checked || selectedDepartments.length > 0) {
      const departmentsToShow = selectAllCheckbox.checked
        ? []
        : selectedDepartments;
      ...
      originalRows.forEach((row) => {
        ...
        if (
          ...
          departmentsToShow.some(
            (dept) => departmentValue.toLowerCase() === dept.toLowerCase()
          )
        ) {
      ...
      if (rowsAdded === 0) {
        ...
        noResultsRow.innerHTML =
          '<td colspan="8">No hay información disponible para los filtros seleccionados</td>';
      ...
      noSelectionRow.innerHTML =
        '<td colspan="8">Seleccione al menos un departamento para ver la información</td>';
  ...
  selectAllCheckbox.addEventListener("change", function () {
    if (this.checked) {
      ...
      departmentCheckboxes.forEach((cb) => {
        cb.checked = previousSelections.has(
  ...
  departmentCheckboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", function () {
      const checkedCount = Array.from(departmentCheckboxes).filter(
        (cb) => cb.checked
      ).length;
  ...
  if (typeof sessionDepartment !== "undefined" && sessionDepartment) {
    // Si es usuario con rol 0 o el departamento es Posgrados, seleccionar todos
    if (userRol === "0" || isPosgrados === "true") {
      selectAllCheckbox.checked = true;
      departmentCheckboxes.forEach((checkbox) => {
      ...
      departmentCheckboxes.forEach((checkbox) => {
        ...
        if (
          checkboxDepartment.toLowerCase() === sessionDepartment.toLowerCase()
        ) {
          checkbox.checked = true;
